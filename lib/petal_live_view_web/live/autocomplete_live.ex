defmodule PetalLiveViewWeb.AutocompleteLive do
  use PetalLiveViewWeb, :live_view
  alias PetalLiveView.Stores
  alias PetalLiveView.Cities
  ...
  def handle_info({:run_city_search, city}, socket) do
    case Stores.search_by_city(city) do
      [] ->
        socket =
          socket
          |> put_flash(:info, "No stores mathcing \"#{city}\"")
  ...
  def handle_info({:run_zip_search, zip}, socket) do
    case Stores.search_by_zip(zip) do
      [] ->
        socket =
          socket
          |> put_flash(:info, "No stores mathcing \"#{zip}\"")
          |> assign(stores: [], loading: false)
        {:noreply, socket}
